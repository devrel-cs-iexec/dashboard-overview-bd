      ...
      <main id="content" className="flex-1 px-6 py-8 lg:px-10 lg:py-10">
        <StatTiles
          stats={[
            {
              label: "Total handles",
              value: totals.handles.toLocaleString(),
              sub: "all-time",
              color: "var(--color-accent)",
            },
            {
              label: "Last 24h",
              value: totals.handlesLast24h.toLocaleString(),
              sub: "new handles",
              color: "var(--color-accent)",
            },
            {
              label: "Last 7d",
              value: totals.handlesLast7d.toLocaleString(),
              sub: "new handles",
              color: "var(--color-accent)",
            },
            {
              label: "Distinct operators",
              value: totals.distinctOperators.toLocaleString(),
              sub: "unique op types",
              color: "var(--color-accent)",
            },
          ]}
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="surface-solid rounded-2xl p-6">
            <h2 className="font-display mb-5 text-[17px] font-medium">
              Operation breakdown
            </h2>
                ...
                return (
                  <div key={cat}>
                    <div className="mb-1 flex items-center justify-between text-[12px]">
                      <span className="text-[var(--color-foreground)]/85">
                        {CAT_LABEL[cat] ?? cat}
                      </span>
                    ...
                    <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/[0.06]">
                      <div
                        ...
                        style={{
                          width: `${pct}%`,
                          background: CAT_COLOR[cat] ?? "var(--color-muted)",
                        }}
                      />
                    </div>
                  </div>
                  ...
                  <div key={op.operator} className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between text-[12px]">
                        <span className="font-mono text-[var(--color-foreground)]/85">
                          {op.operator}
                        </span>
                        <span className="font-mono text-[var(--color-muted)]">
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <span className="font-mono text-[11px] text-[var(--color-muted)]">
                    {t.holderCount > 0 ? `${t.holderCount} holders` : ""}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    {
                      label: "Unwrap scan",
                      value: t.unwrapsScanned
                        ? "Complete"
                        : "Failed — TVS is a lower bound",
                    },
                  ].map((m) => (
                    <div key={m.label}>
                      <div className="text-[10px] uppercase tracking-[0.16em] text-[var(--color-muted-2)]">
                        {m.label}
                      </div>
                      <div className="mt-1 font-mono text-[13px] text-[var(--color-foreground)]/90">
                        {m.value}
                      </div>
                    </div>
                  ))}
