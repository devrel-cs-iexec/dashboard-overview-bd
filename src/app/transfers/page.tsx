...
export default async function TransfersPage() {
  const { items: transfers, online: ponderOnline } = await scanConfidentialTransfers(200);
    ...
    <>
      ...
      <main id="content" className="flex-1 px-6 py-8 lg:px-10 lg:py-10">
        <p className="mb-6 max-w-2xl text-[14px] leading-[1.55] text-[var(--color-muted)]">
          ERC-7984 token events indexed by Ponder. Transfer amounts are encrypted
          (euint256 handle) — only the sender, receiver, and event type are visible
          on-chain.
        </p>
        ...
        {transfers.length === 0 ? (
          <div className="surface-solid rounded-2xl px-8 py-16 text-center">
            {ponderOnline ? (
              <>
                <div className="mb-3 font-mono text-[11px] uppercase tracking-[0.22em] text-blue-400">
                  Ponder syncing
                </div>
                <p className="text-[14px] text-[var(--color-muted)]">
                  The Ponder indexer is running and backfilling on-chain data. Refresh in
                  a few minutes once indexing catches up.
                </p>
              </>
            ) : (
              <>
                <div className="mb-3 font-mono text-[11px] uppercase tracking-[0.22em] text-amber-400">
                  Ponder offline
                </div>
                <p className="text-[14px] text-[var(--color-muted)]">
                  ...
                  in the indexer repo) to populate this feed.
                </p>
              </>
            )}
          </div>
            ...
            <div className="mb-8">
              <StatTiles
                stats={[
                  ...
                  {
                    label: "Mints",
                    value: mintCount.toLocaleString(),
                    color: "var(--color-positive)",
                  },
                  {
                    label: "Burns",
                    value: burnCount.toLocaleString(),
                    color: "var(--color-negative)",
                  },
                  {
                    label: "Transfers",
                    value: xferCount.toLocaleString(),
                    color: "#38bdf8",
                  },
                ]}
              />
                        ...
                        >
                          ...
                          <td className="px-5 py-3 font-mono text-[12px] sm:px-7">
                            {t.from === "0x0000000000000000000000000000000000000000" ? (
                              ...
                              <a
                                href={explorerAddress(t.chainId, t.from)}
                                target="_blank"
                                rel="noreferrer"
                          ...
                          <td className="px-5 py-3 font-mono text-[12px] sm:px-7">
                            {t.to === "0x0000000000000000000000000000000000000000" ? (
                              ...
                              <a
                                href={explorerAddress(t.chainId, t.to)}
                                target="_blank"
                                rel="noreferrer"
                                className="text-[var(--color-foreground)]/85 hover:text-[var(--color-accent)]"
                              >
