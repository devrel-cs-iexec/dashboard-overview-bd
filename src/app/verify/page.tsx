    ...
    <>
      ...
      <main id="content" className="flex-1 px-6 py-8 lg:px-10 lg:py-10">
        <p className="mb-6 max-w-2xl text-[14px] leading-[1.55] text-[var(--color-muted)]">
          Verify that a given handle exists on-chain. Paste the bytes32 handle ID to
          confirm its operation type, timestamp, and public decryptability status.
        </p>
        ...
        {searched && !handle && (
          <div className="surface-solid rounded-2xl px-8 py-16 text-center">
            <div className="mb-3 font-mono text-[11px] uppercase tracking-[0.18em] text-[var(--color-negative)]">
              Not found
            </div>
            <p className="text-[14px] text-[var(--color-muted)]">
              Handle{" "}
              <span className="font-mono text-white">{shortAddress(handleId!)}</span> does
              not exist in the Ponder index.
            </p>
          </div>
        )}
                ...
                <div className="surface-solid overflow-hidden rounded-2xl">
                  {[
                    ...
                    { label: "Category", value: cat },
                    {
                      ...
                      value: handle.isPubliclyDecryptable
                        ? "Yes — anyone can request plaintext"
                        : "No — encrypted only",
                    },
                    {
                      label: "Block timestamp",
                      value: `${handle.blockTimestamp} (${relativeTime(Number(handle.blockTimestamp))})`,
                    },
                    { label: "Transaction", value: handle.transactionHash },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
                      className="grid grid-cols-[200px_1fr] border-b border-[var(--color-border)] px-6 py-4 last:border-0"
                    >
                      <div className="font-mono text-[11px] uppercase tracking-[0.16em] text-[var(--color-muted-2)]">
                        {label}
                      </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="surface-solid rounded-xl p-5">
              ...
              <p className="text-[13px] text-[var(--color-muted)]">
                Every encrypted value in Nox is represented as a bytes32 handle — a unique
                identifier pointing to an encrypted ciphertext managed by the coprocessor.
              </p>
            </div>
            <div className="surface-solid rounded-xl p-5">
              <div className="mb-2 font-mono text-[11px] uppercase tracking-[0.18em] text-[#a78bfa]">
